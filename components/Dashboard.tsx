...
export const Dashboard: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto animate-fadeIn pb-20 md:pb-12">
      ...
      <div className="block md:hidden space-y-4 px-1">
        ...
        <div className="flex flex-col gap-3 px-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-black text-slate-900 tracking-tight uppercase">Visão Geral</h2>
          </div>
          ...
          <div className="flex gap-2 w-full">
            <select 
                ...
                className="flex-1 bg-white border border-slate-200 rounded-xl px-3 py-2.5 text-[10px] font-black uppercase outline-none shadow-sm"
            >
            ...
            <div className="flex-1 relative" ref={dropdownRef}>
                <button 
                    ...
                    className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2.5 text-[10px] font-black uppercase flex items-center justify-between shadow-sm"
                >
                ...
                {showMonthDropdown && (
                    <div className="absolute top-full right-0 mt-2 w-64 bg-white border border-slate-200 rounded-2xl shadow-2xl z-[150] overflow-hidden animate-slideUp">
                        <div className="p-2 grid grid-cols-1 gap-0.5 max-h-60 overflow-y-auto">
                            {monthNames.map((m, i) => (
                                <button 
                                    key={i} 
                                    onClick={() => toggleTempMonth(i + 1)}
                                    className={`flex items-center gap-3 p-2.5 rounded-xl text-[10px] font-bold uppercase transition-colors ${tempSelectedMonths.includes(i + 1) ? 'bg-blue-50 text-blue-600' : 'text-slate-500'}`}
                                >
                        ...
                        <div className="p-3 border-t border-slate-100">
                            <button onClick={handleApplyFilter} className="w-full bg-blue-600 text-white py-2 rounded-xl text-[10px] font-black uppercase">Aplicar</button>
                        </div>
                    </div>
                )}
            </div>
        ...
        <div className="bg-white rounded-[24px] p-5 shadow-sm border border-slate-200">
          <div className="flex justify-between items-start mb-4">
             <div>
                <p className="text-slate-400 text-[8px] font-black uppercase tracking-widest mb-1">Objetivo Planejado</p>
                <p className="text-xl font-black text-slate-900 leading-none">{formatCurrency(data.meta)}</p>
             </div>
             <div className="text-right">
                <p className={`text-xl font-black ${percentualAtingido >= 100 ? 'text-blue-600' : 'text-red-600'}`}>
                  {percentualAtingido.toFixed(1)}%
                </p>
                <p className="text-[7px] text-slate-400 font-black uppercase tracking-widest">Atingimento</p>
             </div>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-end">
              <span className="text-[8px] font-black text-slate-400 uppercase flex items-center gap-1">
                <DollarSign className="w-2.5 h-2.5 text-blue-500" /> Faturado
              </span>
              <span className="text-sm font-black text-slate-900">{formatCurrency(data.faturado)}</span>
            </div>
            <div className="h-2 bg-slate-100 rounded-full overflow-hidden border border-slate-200/50">
              <div 
                className={`h-full transition-all duration-1000 ${percentualAtingido >= 100 ? 'bg-blue-600' : 'bg-red-600'}`} 
          ...
          {data.hasPrevData && (
              <div className={`mt-4 inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[8px] font-black uppercase ${data.growthPercent >= 0 ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                {data.growthPercent >= 0 ? <ArrowUpRight className="w-2.5 h-2.5" /> : <ArrowDownRight className="w-2.5 h-2.5" />}
                {Math.abs(data.growthPercent).toFixed(1)}% vs Ano Anterior
              </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div onClick={() => setShowPositivizedModal(true)} className="bg-white rounded-[24px] p-4 border border-slate-200 shadow-sm active:bg-slate-50 transition-colors">
            <div className="flex justify-between items-start mb-2">
              <div className="p-2 bg-blue-50 text-blue-600 rounded-lg">
                <Users className="w-4 h-4" />
              </div>
              <span className="text-[8px] font-black text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded-md">
                {percentualClientes.toFixed(0)}%
              </span>
            </div>
            <p className="text-slate-400 text-[8px] font-black uppercase tracking-widest mb-1">Positivados</p>
            <div className="flex items-baseline gap-1">
              <h4 className="text-xl font-black text-slate-900">{data.clientesPositivados}</h4>
              <span className="text-[10px] text-slate-400 font-bold">/ {data.totalClientes}</span>
            </div>
          </div>

          <div onClick={() => setShowNonPositivizedModal(true)} className="bg-white rounded-[24px] p-4 border border-slate-200 shadow-sm active:bg-slate-50 transition-colors">
            <div className="flex justify-between items-start mb-2">
            ...
            <p className="text-slate-400 text-[8px] font-black uppercase tracking-widest mb-1">Pendentes</p>
            <h4 className="text-xl font-black text-slate-900">{data.totalClientes - data.clientesPositivados}</h4>
          </div>
        </div>
      ...
      {/* 🖥️ LAYOUT ORIGINAL PARA DESKTOP (PC e Tablets Grandes) */}
