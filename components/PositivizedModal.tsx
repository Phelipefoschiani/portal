      ...
      <div className="bg-white w-full max-w-2xl rounded-[32px] shadow-2xl flex flex-col max-h-[85vh] animate-slideUp overflow-hidden border border-white/20">
        <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-emerald-50/80">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-2xl font-black text-emerald-900 tracking-tight">Positivados</h3>
              <p className="text-xs text-emerald-700 font-bold uppercase tracking-wider">Clientes com faturamento no período</p>
            </div>
          </div>
        ...
        <div className="overflow-y-auto flex-1 bg-white p-4 custom-scrollbar">
          {!totalDataStore.isHydrated ? (
            <div className="flex flex-col items-center justify-center py-24 text-slate-400">
              <Loader2 className="w-10 h-10 animate-spin mb-4 text-emerald-600" />
              <p className="text-sm font-black uppercase tracking-widest animate-pulse text-emerald-700">Calculando faturamentos do período...</p>
            </div>
          ) : list.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-slate-400">
              ...
              <p className="font-black text-slate-800 text-xl tracking-tight">Período sem Vendas</p>
              <p className="text-sm mt-2 font-medium">Nenhuma compra registrada para este filtro ainda.</p>
            </div>
          ) : (
              ...
              {list.map((item, idx) => (
                <div key={idx} className="p-6 rounded-2xl bg-white border border-slate-100 flex items-center justify-between group hover:shadow-xl transition-all hover:border-emerald-200 shadow-sm">
                  <div className="flex-1">
                    ...
                    <p className="text-[10px] text-slate-400 font-bold tracking-wider uppercase">CNPJ: {item.originalCnpj}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-2xl font-black text-emerald-600 leading-none tabular-nums">{formatCurrency(item.total)}</p>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.1em] mt-2">Faturado Acumulado</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="p-6 border-t border-slate-100 bg-slate-50/80 rounded-b-[32px] flex justify-between items-center px-10">
           <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Consolidado da Seleção</span>
           <div className="flex items-center gap-4">
