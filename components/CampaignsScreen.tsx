...
export const CampaignsScreen: React.FC<CampaignsScreenProps> = ({ onNavigateToInvestments }) => {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto space-y-6 animate-fadeIn pb-24 md:pb-20">
      ...
      <div className="md:hidden space-y-5 px-1">
         ...
         <div className="flex flex-col gap-4">
            ...
            <div className="bg-white p-1 rounded-xl border border-slate-200 shadow-sm flex">
                <button 
                    ...
                    className={`flex-1 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'new' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400'}`}
                >
                    {editingId ? 'Revisar' : 'Nova Solicitação'}
                </button>
                <button 
                    onClick={() => setActiveTab('history')} 
                    className={`flex-1 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'history' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400'}`}
                >
                    Histórico
                </button>
            </div>
         ...
         {activeTab === 'new' ? (
             <div className="space-y-5">
                 {/* Mobile KPI Summary Card */}
                 <div className="bg-slate-900 p-5 rounded-[24px] text-white shadow-lg relative overflow-hidden">
                    <div className="absolute right-0 top-0 p-4 opacity-10"><TrendingUp className="w-24 h-24" /></div>
                    <div className="flex justify-between items-end relative z-10">
                        <div>
                            <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest mb-1">Total Solicitado</p>
                            <p className="text-3xl font-black">{formatBRL(totalInvestment)}</p>
                        </div>
                        <div className="text-right">
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">ROI %</p>
                            <p className={`text-xl font-black ${investmentPercent > 7 ? 'text-red-400' : 'text-emerald-400'}`}>
                                {investmentPercent.toFixed(1)}%
                            </p>
                        </div>
                    </div>
                    {investmentPercent > 7 && (
                        <div className="mt-3 bg-red-500/20 px-3 py-1.5 rounded-lg border border-red-500/30 flex items-center gap-2">
                            <AlertCircle className="w-3.5 h-3.5 text-red-400" />
                            <span className="text-[8px] font-black text-red-200 uppercase">ROI Alto • Requer Aprovação</span>
                        </div>
                    )}
                 </div>
                 ...
                 <div className="bg-white p-5 rounded-[24px] border border-slate-200 shadow-sm space-y-5">
                    ...
                    <div>
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Valor do Pedido (R$)</label>
                        <input 
                            type="text"
                            value={orderValue}
                            onChange={handleInputChange(setOrderValue)}
                            placeholder="0,00"
                            className="w-full h-14 bg-slate-50 border border-slate-200 rounded-xl px-4 text-lg font-black text-slate-900 outline-none focus:ring-2 focus:ring-blue-100 tabular-nums"
                        />
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label className="text-[9px] font-black text-emerald-600 uppercase tracking-widest mb-2 block">Cash (R$)</label>
                            <input 
                                type="text"
                                value={dinheiro}
                                onChange={handleInputChange(setDinheiro)}
                                placeholder="0,00"
                                className="w-full h-12 bg-emerald-50/50 border border-emerald-100 rounded-xl px-3 text-sm font-black text-emerald-700 outline-none focus:ring-2 focus:ring-emerald-200 tabular-nums text-center"
                            />
                        </div>
                        <div>
                            <label className="text-[9px] font-black text-blue-600 uppercase tracking-widest mb-2 block">Produto (R$)</label>
                            <input 
                                type="text"
                                value={produto}
                                onChange={handleInputChange(setProduto)}
                                placeholder="0,00"
                                className="w-full h-12 bg-blue-50/50 border border-blue-100 rounded-xl px-3 text-sm font-black text-blue-700 outline-none focus:ring-2 focus:ring-blue-200 tabular-nums text-center"
                            />
                        </div>
                    </div>

                    <div>
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Justificativa</label>
                        <textarea 
                            value={description}
                            onChange={(e) => setDescription(e.target.value)}
                            rows={3}
                            className="w-full p-4 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-600 outline-none focus:ring-2 focus:ring-blue-100"
                    ...
                    <Button 
                        ...
                        className="h-14 rounded-xl font-black text-xs uppercase tracking-widest shadow-xl"
                    >
                        {editingId ? 'Reenviar Solicitação' : 'Enviar Solicitação'}
                    </Button>
                 </div>
             </div>
         ) : (
             <div className="space-y-3">
                 {isLoadingHistory ? (
                     ...
                     <div className="py-20 text-center bg-white rounded-2xl border border-dashed border-slate-200">
                         ...
                         <p className="text-[10px] font-black text-slate-400 uppercase">Sem solicitações</p>
                     </div>
                 ) : (
                     history.map(inv => {
                        ...
                        return (
                            <div key={inv.id} className={`bg-white p-5 rounded-2xl border shadow-sm flex flex-col gap-3 ${isRejected ? 'border-red-200 ring-2 ring-red-50' : 'border-slate-200'}`}>
                                <div className="flex justify-between items-start">
                                    <div className="min-w-0 pr-2">
                                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-0.5">{new Date(inv.data).toLocaleDateString('pt-BR')}</p>
                                        <h4 className="font-black text-slate-800 uppercase text-xs truncate leading-tight">{inv.clientes?.nome_fantasia}</h4>
                                    </div>
                                ...
                                <div className="grid grid-cols-2 gap-2 text-[10px]">
                                    <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                                        <span className="block text-[7px] font-black text-slate-400 uppercase">Pedido</span>
                                        <span className="font-bold text-slate-700">R$ {pedidoValue}</span>
                                    </div>
                                    <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                                        <span className="block text-[7px] font-black text-slate-400 uppercase">Verba</span>
                                        <span className="font-black text-blue-600">{formatBRL(inv.valor_total_investimento)}</span>
                                    </div>
                                </div>

                                {isRejected && (
                                    <div className="mt-1">
                                        <p className="text-[9px] text-red-600 font-bold italic bg-red-50 p-2 rounded-lg mb-2 line-clamp-2">
                                            "{obs.match(/\[RECUSADO:\s*(.*?)\]/)?.[1] || 'Verifique os valores.'}"
                                        </p>
                                        <button 
                                            onClick={() => handleEditRejected(inv)}
                                            className="w-full py-2.5 bg-red-600 text-white rounded-lg text-[9px] font-black uppercase flex items-center justify-center gap-2 shadow-sm active:scale-95 transition-transform"
                                        >
                                            <RefreshCw className="w-3 h-3" /> Corrigir
                                        </button>
                                    </div>
                                )}
                            </div>
