...
const RepBreakdownModal: React.FC<{ 
    ...
    const breakdown = useMemo(() => {
        // Mapa de CNPJ -> Nome (da carteira de clientes) para garantir identificação nominal
        ...
        // Filtrar vendas APENAS deste representante, deste mês e deste ano
        const sales = totalDataStore.sales.filter(s => {
            const d = new Date(s.data + 'T00:00:00');
            return s.usuario_id === rep.id && d.getUTCMonth() === monthIdx && d.getUTCFullYear() === year;
        });

        const repTotal = sales.reduce((a, b) => a + Number(b.faturamento), 0);
        const channelMap = new Map<string, any>();

        sales.forEach(s => {
            // Agrupar por Canal (Distribuição, Alimentar, Farma, etc)
        ...
        return Array.from(channelMap.values())
            ...
            .map(ch => ({
                ...ch,
                shareInRep: repTotal > 0 ? (ch.total / repTotal) * 100 : 0,
    ...
    return createPortal(
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md animate-fadeIn">
            <div className="bg-white w-full max-w-4xl rounded-[40px] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden border border-white/20">
                <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <div>
                        <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter">Decomposição por Canal</h3>
                        <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mt-1">
                            {rep.nome} • {monthNames[monthIdx]} {year}
                        </p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar bg-white">
                    {breakdown.length === 0 ? (
                        <div className="py-20 text-center text-slate-300">
                            ...
                            <p className="font-black uppercase tracking-widest text-[11px]">Nenhum cliente positivado neste período</p>
                        </div>
                    ) : (
                            ...
                            <div key={cIdx} className="space-y-4">
                                <div className="flex items-center justify-between bg-slate-900 text-white p-5 rounded-2xl shadow-lg border-b-4 border-blue-600">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className="text-[9px] font-black text-blue-400 uppercase tracking-widest">{channel.shareInRep.toFixed(1)}% do total</p>
                                    </div>
                                </div>
                                <div className="bg-slate-50 rounded-2xl border border-slate-100 overflow-hidden">
                                    <table className="w-full text-left">
                                        <thead className="bg-slate-100/50 text-[9px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                                            <tr>
                                        ...
                                        <tbody className="divide-y divide-slate-100">
                                            {channel.clients.map((client: any, clIdx: number) => (
                                                <tr key={clIdx} className="hover:bg-white transition-colors">
                                                    <td className="px-6 py-3 text-[11px] font-bold text-slate-700 uppercase truncate max-w-[300px]">{client.name}</td>
                                                    <td className="px-6 py-3 text-right font-black text-slate-900 text-[11px]">{formatBRL(client.total)}</td>
                ...
                <div className="p-4 bg-slate-50 border-t border-slate-100 text-center">
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Exibindo apenas os clientes com faturamento no período selecionado</p>
                </div>
            </div>
        </div>,
        document.body
...
const MonthRepDetailModal: React.FC<{ 
    ...
    const monthNames = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"];
    const [selectedRepForBreakdown, setSelectedRepForBreakdown] = useState<any | null>(null);
    ...
    const data = useMemo(() => {
        ...
        const totalMonthBilling = sales.reduce((a, b) => a + Number(b.faturamento), 0);
        ...
        return reps.map(rep => {
            ...
            // Positivação: Contagem de clientes únicos que compraram este mês
            ...
            return {
                ...
                target,
                positivados,
                share: totalMonthBilling > 0 ? (billed / totalMonthBilling) * 100 : 0
            };
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fadeIn">
            <div className="bg-white w-full max-w-5xl rounded-[40px] shadow-2xl flex flex-col max-h-[90vh] overflow-hidden border border-white/20">
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-100">
                                <tr>
                                    ...
                                    <th className="px-6 py-5 text-right">Meta</th>
                                    <th className="px-6 py-5 text-right">Faturado</th>
                                    <th className="px-6 py-5 text-center">Positiv.</th>
                                    <th className="px-8 py-5 text-right">Part. no Mês (%)</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {data.map((rep, idx) => (
                                    <tr 
                                        key={idx} 
                                        onClick={() => setSelectedRepForBreakdown(rep)}
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 text-right font-bold text-slate-400 text-xs tabular-nums">{formatBRL(rep.target)}</td>
                                        <td className="px-6 py-4 text-right font-black text-slate-900 text-xs tabular-nums">{formatBRL(rep.billed)}</td>
                                        <td className="px-6 py-4 text-center">
                                            <span className="bg-blue-50 text-blue-600 px-2.5 py-1 rounded-lg text-[9px] font-black">{rep.positivados} Clts</span>
                                        </td>
                                        <td className="px-8 py-4">
                                            <div className="flex flex-col items-end gap-1">
                                                <span className="text-[11px] font-black text-blue-600">{rep.share.toFixed(1)}%</span>
                                                <div className="w-16 h-1 bg-slate-100 rounded-full overflow-hidden">
                                                    <div className="h-full bg-blue-600" style={{ width: `${Math.min(rep.share * 2, 100)}%` }}></div>
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                ...
                <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-center">
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Clique em um representante para abrir a decomposição da carteira</p>
                </div>
            </div>

            {selectedRepForBreakdown && (
                <RepBreakdownModal 
                    rep={selectedRepForBreakdown} 
                    monthIdx={monthIdx} 
                    year={year} 
                    onClose={() => setSelectedRepForBreakdown(null)} 
                    formatBRL={formatBRL}
                />
            )}
        </div>,
                ...
                <div className="h-[350px] w-full flex items-end justify-between gap-3 md:gap-6 px-2 pt-10 border-b border-slate-100">
                    {stats.globalPerformance.map((item: any, idx: number) => {
                        ...
                        return (
                            <div 
                                key={idx} 
                                onClick={() => setSelectedMonthForDetail(idx)}
